...
class RandomColorDualParameters(SpeedWithDirectionParameters, ColorInterpolationParameters):
    def __init__(self):
        super().__init__()
    ...
    def run(self, ms: int):
        ...
        if orientation in ["horizontal", "vertical"]:
            self.controller.map_scaled_coordinates(
                lambda x, y, index: Effect.interpolate_color(first_color, second_color, x if orientation == "horizontal" else y, self.PARAMETERS.interpolation.get_value()), force_positive=True)
        elif orientation == "radial":
            self.controller.map_scaled_distance(
                lambda distance, index: Effect.interpolate_color(first_color, second_color, distance, self.PARAMETERS.interpolation.get_value()))
